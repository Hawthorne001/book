...
```bash
$ cast key-auth encode --help
```

```txt
Usage: cast key-auth encode [OPTIONS] --chain-id <CHAIN_ID> <KEY_ADDRESS>
...
Options:
      ...
      --key-type <KEY_TYPE>
          ...
          authorization itself is signed with the configured Ethereum
          (secp256k1) wallet
