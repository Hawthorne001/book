...
Commands:
  ...
  batch-mktx             Build and sign a batch transaction (Tempo) [aliases:
                         bm]
  batch-send             Sign and publish a batch transaction (Tempo) [aliases:
                         bs]
  ...
  keccak                 Hash arbitrary data using Keccak-256 [aliases: k,
                         keccak256]
  key-auth               Tempo key authorization RLP helpers
  keychain               Tempo keychain (access key) management [aliases: kc]
  ...
  tip20-token            TIP-20 token operations (Tempo) [aliases: tip20]
